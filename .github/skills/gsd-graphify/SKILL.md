...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-tools query`. Use `node $HOME/.copilot/gsd-core/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-tools query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node $HOME/.copilot/gsd-core/bin/gsd-tools.cjs config-set graphify.enabled true
...
```bash
node $HOME/.copilot/gsd-core/bin/gsd-tools.cjs graphify query <term>
```
...
```bash
node $HOME/.copilot/gsd-core/bin/gsd-tools.cjs graphify status
```
...
```bash
node $HOME/.copilot/gsd-core/bin/gsd-tools.cjs graphify diff
```
...
```bash
node "$HOME/.copilot/gsd-core/bin/gsd-tools.cjs" graphify build
```
...
graphify update . \
  ...
  && node "$HOME/.copilot/gsd-core/bin/gsd-tools.cjs" graphify build snapshot \
  && node "$HOME/.copilot/gsd-core/bin/gsd-tools.cjs" graphify status
```
