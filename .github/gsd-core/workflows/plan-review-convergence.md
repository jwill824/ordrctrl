...
```bash
INIT=$(node ".github/gsd-core/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node ".github/gsd-core/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node ".github/gsd-core/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
