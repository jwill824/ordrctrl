...
<task type="auto" tdd="true">
  ...
  <verify>
    ...
    <automated>cd frontend && npm test -- tests/unit/components/BottomTabBar.test.tsx tests/unit/components/Sidebar.test.tsx 2>&1 | tail -5</automated>
  </verify>
  <done>
...
<verification>
Plan 01 is complete when ALL of the following pass:

```bash
# 1. Typecheck clean
cd frontend && npm run typecheck

# 2. New component tests GREEN
...
# 3. AppShell uses Outlet
cd frontend && grep -c "Outlet" src/components/AppShell.tsx
# Expected: >= 1

# 4. IntegrationSettingsContent is exported
...
Continue with Plan 02 (`07-02-PLAN.md`) for page chrome strip + FAB polish + human verify.
</output>
