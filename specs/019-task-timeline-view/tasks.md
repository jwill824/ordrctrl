...
- [x] T005 [P] [US1] Implement `useTimeline` hook in `frontend/src/hooks/useTimeline.ts` — import `toLocalMidnight` from `frontend/src/utils/dateUtils.ts` for timezone-safe midnight normalization; `dateKey = dueAt ?? startAt` (fall back to `startAt` for calendar events with null `dueAt`); filter out `completed` and `dismissed` items; sort each bucket by `dueAt ?? startAt ASC` then `title ASC` (unscheduled by `title ASC` only); return only non-empty `TimelineGroupData[]` in canonical order: overdue → today → this-week → later → unscheduled; accept `FeedItem[]` and `now: Date` as inputs
...
- All `diffDays` calculations **MUST** use `toLocalMidnight()` from `frontend/src/utils/dateUtils.ts` — do NOT call `setHours(0,0,0,0)` directly on dates from external sources. `toLocalMidnight` detects all-day sentinels (UTC midnight `T00:00:00Z` and UTC noon `T12:00:00Z`) and extracts UTC date components to avoid timezone shift in UTC-negative locales. See `data-model.md` for the canonical algorithm.
