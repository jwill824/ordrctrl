...
- [x] T013 [US4] Create `.github/workflows/native.yml` with `on:` trigger (`push`/`pull_request` to `main`, `paths: ['frontend/**']`) and `build-ios` job: `macos-latest`, `timeout-minutes: 40`, no external `needs:` (native jobs self-contain `pnpm --filter frontend build`), `xcodebuild -sdk iphonesimulator -configuration Debug`, upload `frontend/ios/build/App.app` artifact as `ios-simulator-app` (FR-015, FR-017, FR-018, per ci-jobs-contract.md Job 2)
- [x] T014 [US4] Add `build-android` job to `.github/workflows/native.yml`: `ubuntu-latest`, `timeout-minutes: 25`, `setup-java@v4` (temurin JDK 17), Gradle cache, `cap sync android`, `./gradlew assembleDebug`, upload `app-debug.apk` artifact as `android-debug-apk` (FR-016, FR-017, FR-018, per ci-jobs-contract.md Job 3)
- [x] T015 [US4] Add `maestro-ios` job to `.github/workflows/native.yml`: `macos-latest`, `timeout-minutes: 30`, `needs: [build-ios]`, download `ios-simulator-app` artifact, install Maestro CLI, boot iOS 17 simulator via `xcrun simctl`, install app, run `maestro test .maestro/flows/` with credential skip guard (FR-014, per ci-jobs-contract.md Job 4)
- [x] T016 [US4] Add `maestro-android` job to `.github/workflows/native.yml`: `ubuntu-latest`, `timeout-minutes: 40`, `needs: [build-android]`, download `android-debug-apk` artifact, install Maestro CLI, `reactivecircus/android-emulator-runner@v2` (API 33, `google_apis`), `adb install`, `maestro test .maestro/flows/` with credential skip guard (FR-014, per ci-jobs-contract.md Job 5)
